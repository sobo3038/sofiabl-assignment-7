...
app = Flask(__name__)
app.secret_key = "S15B\M1B\S26B\M23L\E20B"  # Replace with your own secret key, needed for session management
...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    for _ in range(S):
        ...
        Y_sim = beta0 + beta1 * X_sim.flatten() + mu + np.random.normal(0, np.sqrt(sigma2), N)
...
def hypothesis_test():
    ...
    plt.axvline(hypothesized_value, color="blue", linewidth=2, label=f"Hypothesized {parameter.capitalize()} (H₀): {hypothesized_value}")
...
def confidence_interval():
    ...
    t_critical = t.ppf(1 - alpha / 2, df=S - 1)  
    ...
    plt.axvline(mean_estimate, color="blue", linewidth=2, label="Mean Estimate")
    plt.hlines(0, ci_lower, ci_upper, color="blue", linestyle="-", linewidth=3, label=f"{confidence_level}% Confidence Interval")
